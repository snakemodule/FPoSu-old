...
void OsuBeatmapDifficulty::buildStandardHitObjects(OsuBeatmapStandard *beatmap, std::vector<OsuHitObject*> *hitobjects)
{
	...
	for (int i=0; i<hitcircles.size(); i++)
	{
		...
		//----------Squish
		//c->y = clamp<int>(lerp((float)c->y, (float)OsuGameRules::OSU_COORD_HEIGHT/2, 0.5f) / 8, 0, OsuGameRules::OSU_COORD_HEIGHT);
		//float proportion = ((float)c->y/(float)OsuGameRules::OSU_COORD_HEIGHT) - 0.5f;
		//debugLog("prop: %f\n", proportion);
		//float offset = proportion * (OsuGameRules::OSU_COORD_HEIGHT/2);
		//c->y = (OsuGameRules::OSU_COORD_HEIGHT/2) + offset;
