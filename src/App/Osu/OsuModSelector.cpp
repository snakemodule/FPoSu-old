...
OsuModSelector::OsuModSelector(Osu *osu) : OsuScreen()
{
	...
	OVERRIDE_SLIDER playfieldDistance = addOverrideSlider("Playfield Edge Distance", " ", convar->getConVarByName("fposu_distance"), 0.0f, 2.0f);
	...
	playfieldDistance.slider->setChangeCallback( fastdelegate::MakeDelegate(this, &OsuModSelector::onOverrideSliderChange) );
	playfieldDistance.slider->setValue(1.5f, false);
	playfieldDistance.slider->setAnimated(false); // same quick fix as above
	playfieldDistance.slider->setKeyDelta(0.01);
	...
	m_distanceSlider = playfieldDistance.slider;
		...
		if (m_overrideSliders[i].slider == slider)
		{
			...
			else
			{
				...
				if (m_overrideSliders[i].desc->getText().find("Speed") != -1)
					sliderValue = std::max(sliderValue, 0.05f);
				// HACKHACK: force distance slider to have a min value of 0.01 instead of 0 (because less is literally unplayable) -CB
			}
			if (m_overrideSliders[i].desc->getText().find("Distance") != -1) {
				sliderValue = std::max(sliderValue, 0.01f);
			}
