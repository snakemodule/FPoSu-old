...
struct HighPrecVector2
{
    double long x;
    double long y;

    // ctors
    HighPrecVector2() : x(0), y(0) {};
    HighPrecVector2(double long x, double long y) : x(x), y(y) {};

    // utils functions
    /*void 		zero();
    void        set(double long x, double long y);
    float       length() const;                         //
    float       distance(const Vector2& vec) const;     // distance between two vectors
    Vector2&    normalize();                            //
    float       dot(const Vector2& vec) const;          // dot product
    ...
    // operators
    /*HighPrecVector2     operator-() const;                      // unary operator (negate)
    HighPrecVector2     operator+(const HighPrecVector2& rhs) const;    // add rhs
    HighPrecVector2     operator-(const HighPrecVector2& rhs) const;    // subtract rhs
    HighPrecVector2&    operator+=(const HighPrecVector2& rhs);         // add rhs and update this object
    HighPrecVector2&    operator-=(const HighPrecVector2& rhs);         // subtract rhs and update this object
    HighPrecVector2     operator*(const double long scale) const;     // scale
    HighPrecVector2     operator*(const HighPrecVector2& rhs) const;    // multiply each element
    HighPrecVector2&    operator*=(const double long scale);          // scale and update this object
    HighPrecVector2&    operator*=(const HighPrecVector2& rhs);         // multiply each element and update this object
    HighPrecVector2     operator/(const double long scale) const;     // inverse scale
    HighPrecVector2&    operator/=(const double long scale);          // scale and update this object
    bool        operator==(const HighPrecVector2& rhs) const;   // exact compare, no epsilon
    bool        operator!=(const HighPrecVector2& rhs) const;   // exact compare, no epsilon
    bool        operator<(const HighPrecVector2& rhs) const;    // comparison for sort
    double long       operator[](int index) const;            // subscript operator v[0], v[1]
    double long&      operator[](int index);                  // subscript operator v[0], v[1]

    friend HighPrecVector2 operator*(const float a, const Vector2 vec);
    friend std::ostream& operator<<(std::ostream& os, const Vector2& vec);*/
};
